...
ARTIFACTS=/work/frink/deyoung.j/demo/rrnlp_demo/expts_tiab_extraction
OUTPUTS=$ARTIFACTS
mkdir -p $ARTIFACTS
FAILED_FILE=$ARTIFACTS/failed2
rm -f $FAILED_FILE
#MAX_JOBS=2
...
output_json="/work/frink/deyoung.j/demo/titles_abstracts2.json"
...
#    #sleep 1s
#    #cmd="srun -p 177huntington -J $(basename $nxml) --mem 48G \
#    cmd="srun -p short -J $(basename $nxml | sed -e 's/pubmed//') --mem 4G \
#    $PYTHON /work/frink/deyoung.j/demo/rrnlp_demo/scripts/demo/scripts/demo/extract_title_abstracts.py \
#        --input_gzs $nxml \
#        --output_json $OUTPUTS/$(basename $nxml).json \
#    " 
...
cmd="srun -p frink --mem 64G \
    $PYTHON /work/frink/deyoung.j/demo/rrnlp_demo/scripts/demo/scripts/demo/merge_title_abstracts.py \
        --input_jsons $OUTPUTS/*json \
        --output_json $output_json \
        --output_df $output_json.csv \
    "
ckpt "$cmd" "merge_tiab"
