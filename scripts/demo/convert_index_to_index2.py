...
m=16 # bits per vector, code size, is typically a power of 2 between 2 and 64
# https://github.com/facebookresearch/faiss/wiki/Faiss-indexes#cell-probe-methods-indexivf-indexes
nlist=2**10 # number of cells (centroids)
target_index = faiss.IndexIVFPQ(quantizer, d, nlist, m, 16)
...
# test
faiss.write_index(target_index, output_index_file)
print('training')
...
#    #res = client.insert(collection_name="pubmed_contriever", data=data)
print('saving')
#target_index.write_index(output_index_file)
faiss.write_index(target_index, output_index_file)
