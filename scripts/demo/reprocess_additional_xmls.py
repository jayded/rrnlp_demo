...
def main():
    ...
    parser.add_argument('--input_jsonl', required=True, help='jsonl input of documents that previously (mysteriously) did not have an abstract in the nxmls but do in pubmed itself')
    ...
    with open(args.input_jsonl, 'r') as inf:
        for line in inf:
            contents = json.loads(line)
            if contents['abstract'] is None or len(contents['abstract'].strip()) == 0:
                continue
            ...
            publication_types = contents['publication_types']
            journal = contents['journal']
            pubdate = contents['pubdate']
